...
Scfdma.N_scSymb     = 14;   % 1 slots with 7 SCFDMA-Symbols

% Channel parameters
Channel.type        =  'ITU-PA';
Channel.N_snapshot  = 9000;
Channel.N_sim       = 90; % Number of snapshots to be simulated
...
for i_ebNo = 1:length(EbNo)
    ...
    for i_ch = 1:Channel.N_sim       
        ...
        % tx{1}  = reshape(repmat(tx{1},3,1),1,[]);
        ...
        for i_user = 1:N_user
            ...
            for i_bl = 0:Scfdma.N_scSymb-1
                ...
                Tx(i_bl*M+(1:M)) = fft(tx_temp(i_bl*M+(1:M)),M)./sqrt(M);
        ...
        for i_user = 1:N_user
            ...
            for i_bl = 0:Scfdma.N_scSymb-1
                ...
                Rx(i_bl*M+(1:M)) = F .* Rx_x(i_bl*N + nu_0+(1:M));
                
                % Equalization
                rx_temp(1,i_bl*M+(1:M)) = ifft(Rx(i_bl*M+(1:M)),M).*(sqrt(M));
