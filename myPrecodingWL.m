function [ V_new, G_all ] = myPrecodingWL( H_all, H_ch, VarN, N, WL )
...
for idx = 1:N
    for k_user = 1:N_user
        ...
        G_all{k_user}(:,:,idx) = pinv(sum_G{k_user}(:,:,idx) + (VarN(k_user)/2)*eye(2*Nr)) * H_all{k_user, k_user}(:,:,idx) * V_init{k_user}(:,:,idx) * Rs{k_user}(:,:,idx);
    end
end
...
for idx = 1:N
    for j = 1:iterations
        ...
        for k_user = 1:N_user
            ...
            sumV_temp{k_user}(:,:,idx) = zeros(Dr, Dr);
            for j_user = 1:N_user
        ...
        for k_user = 1:N_user
            ...
            elseif V_tempnorm{k_user}(1,idx) > 1
                sum_lambda{k_user}(:,:,idx) = zeros(Dr, Dr);
                for j_user = 1:N_user
        ...
        for k_user = 1:N_user
            ...
            G_all{k_user}(:,:,idx) = pinv(sum_G_new{k_user}(:,:,idx) + (VarN(k_user)/2)*eye(2*Nr)) * H_all{k_user, k_user}(:,:,idx) * V_new{k_user}(:,:,idx) * Rs{k_user}(:,:,idx);
        end
        ...
        for k_user = 1:N_user
            ...
            MSE = ((G_all{k_user}(:,:,idx)' * H_all{k_user, k_user}(:,:,idx) * V_new{k_user}(:,:,idx)) - eye(Br)) + sum_MSE{k_user}(:,:,idx) + (G_all{k_user}(:,:,idx)' * G_all{k_user}(:,:,idx) * (VarN(k_user)/2));
            epslon{k_user}(:,:) = MSE * Rs{k_user}(:,:,idx) * MSE';
