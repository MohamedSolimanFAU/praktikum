...
for idx = 1:N
    for i_user = 1:N_user
        for j_user = 1:N_user
            sum_G{i_user}(:,:,idx)  = sum_G{i_user}(:,:,idx) + H_ch{i_user, j_user}(:,:,idx)*V_init{j_user}(:,:,idx)*V_init{j_user}(:,:,idx)'*H_ch{i_user, j_user}(:,:,idx)';
        end
    ...
    for i_user = 1:N_user
        for j_user = 1:N_user
            sum_V{i_user}(:,:,idx)  = sum_V{i_user}(:,:,idx) + H_ch{j_user, i_user}(:,:,idx)'*G_init{j_user}(:,:,idx)*G_init{j_user}(:,:,idx)'*H_ch{j_user, i_user}(:,:,idx);
        end
    ...
    for i_user = 1:N_user
        ...
        for j_user = 1:N_user
            sum_G{i_user}(:,:,idx)  = sum_G{i_user}(:,:,idx) + H_ch{i_user, j_user}(:,:,idx)*V{j_user}(:,:,idx)*V{j_user}(:,:,idx)'*H_ch{i_user, j_user}(:,:,idx)';
        end
